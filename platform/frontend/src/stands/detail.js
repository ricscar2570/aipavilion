...
class StandDetailManager {
    ...
    async loadRelatedStands(standId, category) {
        try {
            ...
            // Fetch all stands and filter by same category, excluding current stand
            const data = await apiService.get('/stands');
            const all  = Array.isArray(data) ? data : (data.stands || []);
            const stands = all
                .filter(s => s.stand_id !== standId && (!category || s.category === category))
                .slice(0, 4);
            ...
            container.innerHTML = stands.map(s => renderStandCard(s, { compact: true })).join('');
            // Wire click handlers after inserting HTML
            container.querySelectorAll('.related-stand-card').forEach(el => {
                el.style.cursor = 'pointer';
                el.addEventListener('click', () => {
                    standDetailManager.loadStandDetails(el.dataset.standId);
                });
            });
