...
export function renderStandCard(stand, { compact = false } = {}) {
    ...
    const booth = stand.booth_number
        ? `<span class="booth">📍 ${escapeHtml(String(stand.booth_number))}</span>`
        : "";
    const rating = stand.rating
        ? `<span class="rating">⭐ ${escapeHtml(String(stand.rating))}</span>`
        : "";
    ...
    return `
        <div class="stand-card" data-testid="stand-card" data-stand-id="${id}">
