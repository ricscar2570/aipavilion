...
import { publicEvents } from "./events/public.js";
...
class AIPavilion {
    ...
    _bindAuthEvents() {
        ...
        document
            ...
            .addEventListener("click", () => this._hideAuthModal());
    ...
    _updateAuthUI() {
        ...
        if (this._currentUser) {
            const email =
                this._currentUser.attributes?.email ||
    ...
    _bindLoginForm() {
        ...
        document
            .getElementById("auth-submit")
            ?.addEventListener("click", () => this._submitAuthForm());
        document
            .getElementById("auth-modal-body")
    ...
    _showUserMenu(anchorEl) {
        ...
        menu.id = "user-menu";
        menu.className =
            "absolute right-4 top-14 glass-card rounded-xl shadow-lg py-2 z-50 min-w-[180px]";
        menu.innerHTML = userMenuHTML();
        document.body.appendChild(menu);

        document
            .getElementById("user-menu-logout")
    ...
    _updateCartBadge() {
        ...
        const count =
            cartManager.getItemCount?.() || cartManager.getCart().length;
        if (count > 0) {
    ...
    _handleRoute() {
        ...
        if (hash === "/search") {
            return this._renderSearch(content);
        }
        if (hash === "/events") {
            return publicEvents.renderList(content);
        }
        if (hash.startsWith("/event/")) {
            const eventId = decodeURIComponent(hash.split("/")[2] || "");
            return eventId
                ? publicEvents.renderDetail(content, eventId)
                : (content.innerHTML = notFoundHTML());
        }
        if (hash === "/cart") {
            return this._renderCart(content);
        }
        if (hash === "/checkout") {
            return this._renderCheckout(content);
        }
        if (hash === "/dashboard") {
            return this._renderDashboard(content);
        }
        if (hash === "/organizer") {
            return this._renderOrganizerPortal(content);
        }
        if (hash === "/exhibitor") {
            return this._renderExhibitorPortal(content);
        }
        if (hash.startsWith("/invitation/")) {
            const invitationId = decodeURIComponent(hash.split("/")[2] || "");
            return this._renderInvitation(content, invitationId);
        }
        if (hash.startsWith("/order-success/")) {
            const orderId = decodeURIComponent(hash.split("/")[2] || "");
            return this._renderOrderSuccess(content, orderId);
        }
        if (hash === "/login") {
            return this._handleLoginRoute();
        }
    ...
    async _loadStands() {
        ...
        try {
            ...
            if (grid) {
                grid.innerHTML =
                    '<p class="text-white/70 col-span-3 text-center py-12">Unable to load stands. Please try again later.</p>';
            }
        }
    }
    ...
    _renderStandCards(stands) {
        ...
        if (!stands.length) {
            grid.innerHTML =
                '<p class="text-white/70 col-span-3 text-center py-12">No stands available yet.</p>';
            return;
        }
    ...
    _renderSearch(content) {
        content.innerHTML =
            '<div id="search-root" class="max-w-7xl mx-auto px-4 py-8"></div>';
    ...
    _renderCheckout(content) {
        if (!this._currentUser) {
            ...
            document
                .getElementById("checkout-login-btn")
                .addEventListener("click", () => this._showLoginModal());
            return;
        }
    ...
    async _initCheckoutForm() {
        try {
            ...
            if (CONFIG.payments.mode === "stripe") {
                const { stripeService } = await import("./checkout/stripe.js");
                ({ cardElement } =
                    await stripeService.createCardElement("#card-element"));
            } else if (CONFIG.payments.mode === "simulated") {
                document.getElementById("card-element").innerHTML =
                    '<p class="text-sm text-amber-700">Development payment simulation: no card data is requested.</p>';
            } else {
            ...
            document
                ...
                .addEventListener("click", async () => {
                    ...
                    try {
                        const { orderId } =
                            await checkoutManager.processCheckout(cardElement);
    ...
    async _renderOrganizerPortal(content) {
        if (!this._currentUser) {
            content.innerHTML = authGateHTML("organizer-login-btn");
            document
                .getElementById("organizer-login-btn")
                .addEventListener("click", () => this._showLoginModal());
            return;
        }
        await tenantPortal.renderOrganizer(content);
    }

    async _renderExhibitorPortal(content) {
        if (!this._currentUser) {
            content.innerHTML = authGateHTML("exhibitor-login-btn");
            document
                .getElementById("exhibitor-login-btn")
                .addEventListener("click", () => this._showLoginModal());
            return;
        }
        await tenantPortal.renderExhibitor(content);
    }

    async _renderInvitation(content, invitationId) {
        if (!invitationId) {
            content.innerHTML = notFoundHTML();
            return;
        }
        if (!this._currentUser) {
            content.innerHTML = authGateHTML(
                "invitation-login-btn",
                "Sign in with the invited email address to accept this invitation.",
            );
            document
                .getElementById("invitation-login-btn")
                .addEventListener("click", () => this._showLoginModal());
            return;
        }
        await tenantPortal.renderInvitation(content, invitationId);
    }

    async _renderOrderSuccess(content, orderId) {
        if (!this._currentUser || !orderId) {
            content.innerHTML = notFoundHTML();
            return;
        }

        content.innerHTML =
            '<div class="max-w-2xl mx-auto px-4 py-16 text-center text-white">Loading order…</div>';
        try {
            const order = await apiService.getOrder(orderId);
            content.innerHTML = orderSuccessHTML(order);
        } catch {
            content.innerHTML = `
                <div class="max-w-2xl mx-auto px-4 py-16 text-center">
                    <div class="glass-card rounded-2xl p-8">
                        <h1 class="text-2xl font-bold text-gray-900 mb-3">Payment received</h1>
                        <p class="text-gray-600 mb-6">We could not load the order details yet. Check your dashboard in a moment.</p>
                        <a href="#/dashboard" class="btn-primary px-6 py-3">Open dashboard</a>
                    </div>
                </div>`;
        }
    }
    ...
    async _renderDashboard(content) {
        if (!this._currentUser) {
            ...
            document
                .getElementById("dash-login-btn")
                .addEventListener("click", () => this._showLoginModal());
            return;
        }
        content.innerHTML =
            '<div id="dashboardContainer" class="max-w-7xl mx-auto px-4 py-8"></div>';
    ...
    _renderStandDetail(content, standId) {
        ...
        content.innerHTML =
            '<div id="standDetailContainer" class="max-w-7xl mx-auto px-4 py-8"></div>';
