/**
 * GET /stands/{standId}  — Single stand detail
 *
 * Fixes applied:
...
const handler = async (event) => {
    ...
    const standId = event.pathParameters?.standId || event.pathParameters?.id;
    ...
    try {
        ...
        return {
            statusCode: 200,
            headers: corsHeaders(event),
            body: JSON.stringify(toPublicStand(result.Item)),
        };
    } catch (error) {
...
exports.handler = withObservability("get-stand-detail", handler);
