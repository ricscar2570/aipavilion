...
const handler = async (event) => {
    ...
    const queryText = rawQuery.slice(0, MAX_QUERY_LENGTH).toLowerCase();
    const eventId = String(params.eventId || "")
        .trim()
        .slice(0, 120);
    const limit = parseLimit(params.limit, 100, 250);
        ...
        const filtered = (result.Items || [])
            ...
            .filter((item) => {
                const tags = Array.isArray(item.tags)
                    ? item.tags.join(" ")
                    : "";
                return [item.name, item.category, item.description, tags]
                    .filter(Boolean)
                    .join(" ")
                    .toLowerCase()
                    .includes(queryText);
